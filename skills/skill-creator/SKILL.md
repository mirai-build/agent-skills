---
name: skill-creator
description: Codex App 用の Skill を新規作成または更新するためのガイド。再利用できる手順、スクリプト、参照資料、アセットを整理し、Skill 本文と UI メタデータを整備したいときに使う。Skill 作成、Skill 更新、社内向け Skill 化、Skill テンプレート整備、Skill の運用改善を依頼されたときに使う。
---
...
3. 出力先を決める。社内共有リポジトリへ入れるなら対象リポジトリの `.agents/skills/`、個人の Codex 環境へ自動検出させるなら `~/.agents/skills` を使う。
...
- `description` は一覧での発見用テキストとして扱い、冒頭は「何をする Skill か」を平文の日本語で書く。
- `description` の冒頭を他 Skill 名、ファイルパス、コマンド、Markdown 記法から始めない。詳細な参照やコード風表記は本文へ回す。
- 本文は命令形または作業指示として書く。
...
- frontmatter が正しいか
- `name` が命名規則に沿っているか
- `description` が十分具体的で、一覧発見用の平文としても安定しているか
