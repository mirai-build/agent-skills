...
- `implementation/openapi.yml` はあるか。あるなら、対象 bounded context の tag から route、request / response、error response、security はそこから読めるか。
- `implementation/NN_<bounded-context>/02_database` はあるか。あるなら、table、制約、監査項目、relation はそこから読めるか。
- `implementation/NN_<bounded-context>/03_async_contracts` はあるか。あるなら、topic / endpoint、payload、認証 / 署名、冪等性、再送方針はそこから読めるか。
...
- `implementation/NN_<bounded-context>/02_database` の table 設計で足りない検索・集計要件はあるか。
...
- `implementation/openapi.yml` の対象 bounded context tag で未確定の request / response schema や example はどこか。
...
- `implementation/NN_<bounded-context>/03_async_contracts` がない場合、どの契約を今回 concrete に決める必要があるか。
