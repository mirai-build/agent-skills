...
- `packages/utils`: domain や layer の文脈を持たない純粋な共通 utility。
- `apps/api`: API runtime の入口。NestJS 固有のルールは `references/profiles/typescript/nest.md` を正本として扱う。
...
- TypeScript の型定義は必要最小限にとどめ、実装前に `packages/core` の domain 型 / Repository interface / shared contract、`packages/db` の generated type、`apps/api` の既存 DTO を棚卸しする。
- 既存の export で意味を表せるならそれを流用し、同じ構造を別名の `FooInput` `BarResult` `BazRecord` として増やさない。
- 新しい型を作るのは、layer 境界を守るために表現を分ける必要があるときか、複数箇所で同じ意味の契約を共有するときだけにする。単発の引数束ねや 1 回しか使わない object literal のためだけに共通型を新設しない。
- 共通化は「形が似ているか」ではなく「意味が同じか」で判断し、所有者が明確な layer に置く。bounded context や layer の意味を持つ型は最も狭い `shared/` に、複数 context から同じ意味で使う契約だけを `packages/core/src/shared` に置く。

## TypeScript ファイル命名規約

- TypeScript のファイル名に snake-case は使わず、CamelCase 系で統一する。
- クラス、ValueObject、Entity、Aggregate、Repository、DTO、Presenter、Builder、model など主語を表すファイルは PascalCase にし、`HogeHoge.ts` `HogeHoge.test.ts` `FugaFuga.models.ts` のように命名する。
- 関数や helper が主役のファイルは lowerCamelCase にし、`someFunction.ts` のように命名する。
- `.test.ts` `.models.ts` `.dto.ts` のような suffix を付ける場合も、suffix の前に置く主語は PascalCase または lowerCamelCase を保ち、snake-case へ崩さない。
- 既存 repo に別の TypeScript 命名規約がある場合でも、snake-case を増やさず、既存の CamelCase 系へ寄せる。
...
- `apps/api` は transport 入口と runtime 配線に集中し、詳細な NestJS ルールは `references/profiles/typescript/nest.md` に従う。
- API を含む use case では、`packages/core` `packages/db` `apps/api` の必要な component を実装し切ってから、DB 接続付き API 結合テストを `references/api_integration_testing.md` に従って追加する。
...
- ユニットテストは対象コンポーネントのディレクトリ直下にある `__tests__/` へ必ず置き、共有の test ディレクトリへ分散させない。
- 大きなファイルを避け、原則 1 ファイル 1 関数、1 ファイル 1 型、1 ファイル 1 責務で分離する。1 つのファイルへ多数の関数、型、補助処理を押し込まない。
- TypeScript のファイル名は snake-case を避け、主語を表すファイルは PascalCase、関数中心のファイルは lowerCamelCase へそろえる。
- 型定義は既存の ValueObject、Entity、service の result 型、shared contract で足りるかを先に確認し、同義の alias や consumer 専用 type を安易に増やさない。
...
- `services` は write side の完了点を担う ApplicationService を置く。
- `queries` は read side の read model やアプリケーション向け出力データを組み立てる QueryService を置く。HTTP request / response DTO は `apps/api` に置く。
- ApplicationService / QueryService の主役ファイルには、対応する `contexts/.../05_services/...md` の設計書パスを短い日本語コメントで残す。
...
- 文字列・数値・日付の単純変換のように domain や layer の文脈を持たない純粋な汎用関数は `packages/utils` に置く。`shared/` は bounded context や layer に意味を持つ共有処理だけに使い、`SomeApplicationService.ts` や `SomeQueryService.ts` に汎用 utility を直書きしない。
- `normalizeRequiredSlug` のような normalize / validation helper を個別コンポーネントの主役ファイルへベタ書きしない。対象 bounded context 全体で共有する helper は `src/<bounded-context>/shared/`、複数 bounded context で使える純 technical helper は `packages/utils` に置き、配置先を責務で分ける。
...
- Prisma 実装は `SomePrismaRepository` のように `packages/core` で先に定義した interface を `implements` する concrete class とし、`mappers/` `helpers/` `errors/` `__tests__/` `index.ts` も同じディレクトリ配下へまとめる。
- ユニットテストは対象 Repository ディレクトリ直下の `__tests__/` へ必ず置き、Repository ごとの責務から離れた共通 test 置き場へ逃がさない。
- Repository 配下でも 1 ファイル 1 責務を崩さず、型、mapper、変換 helper、保存処理を巨大な 1 ファイルへ混在させない。
- Repository class の各関数は永続化依存の接続と処理の流れに集中させ、where 句の構築、record 変換、エラー解釈、保存前後の整形のうち、再利用価値があるものだけを helper / mapper へ切り出す。
- `packages/db` で domain rule や Repository interface を再定義せず、永続化詳細の吸収と core 抽象の実装に責務を限定する。
- Prisma の generated type や `packages/core` の domain 型 / 契約で表現できる範囲は流用し、db 都合の mirror type や interface 再定義を増やさない。
...
- それでも Repository ファイルが太る場合は、aggregate 境界、保存責務、query 系統のどれを混在させているかを見直し、必要なら Repository 自体を分割する。
- DB 固有の test-support は `src/test-support` に置く。

### `apps/api`

- `apps/api` は runtime 全体の入口と composition root を担う。NestJS 固有の tree、命名、Controller / DTO / Presenter / Builder / Mapper のルールは `references/profiles/typescript/nest.md` を正本として扱う。
- `apps/api` は HTTP 境界、webhook / batch / worker 入口、DI 配線、error の HTTP 変換に集中し、業務ロジック本体は持ち込まない。
- DTO や Builder / Presenter の入力型は transport 境界を明確にするために必要なものだけ定義し、既存 DTO や `packages/core` の result 型で足りる場面では流用する。
- `apps/api` の unit test も対象コンポーネントに最も近いディレクトリ直下の `__tests__/` へ必ず置き、別置きの共通 test フォルダへ逃がさない。
- DB 接続付き API 結合テストは unit test と分け、NestJS では `@nestjs/testing` と `supertest` を使う。既存 repo の結合テスト harness を優先し、なければ `api/tests/http-integration/` 配下へ置く。詳細は `references/api_integration_testing.md` を正本にする。
- API 配下の TypeScript ファイル名も snake-case を避け、`AuthenticationController.ts` `CreateUser.dto.ts` `listUsersQuery.ts` のように CamelCase 系でそろえる。
...
- ApplicationService や QueryService の class 関数も、ユースケースの流れと判断箇所が class を読めば追える薄い入口に保つ。
...
- unit test は対象コンポーネントのディレクトリ直下にある `__tests__/` に必ず置き、fake repository で業務ルールを確認する。
- 共有 helper を増やすときは、最初に「その helper は context 固有か、repo 全体で純 technical に再利用できるか」を判断し、前者は `src/<bounded-context>/shared/`、後者は `packages/utils` へ配置する。主役ファイルの末尾へ安易にベタ書きして済ませない。
...
- 複数 Repository で共有する変換のうち永続化文脈を持つものは `src/repositories/shared/` に寄せ、domain や DB に閉じない純粋 utility は `packages/utils` に置く。
- `src/repositories/index.ts` と package の公開面を揃える。

3. `apps/api`
- `references/profiles/typescript/nest.md` を正本にして、global な Nest 設定と context ごとの module 配線を分ける。
- controller / handler は `packages/core` の use case を呼ぶだけの薄い入口に保ち、response DTO は Presenter / Builder / Mapper で表現する。
...
5. API 結合テスト
- API を実装対象に含む場合は、backend component 一式がそろってから `references/api_integration_testing.md` に従って DB 接続付き API 結合テストを追加する。
- 既存の結合テスト script や harness があればそれを拡張し、なければ `api/tests/http-integration/` を最小構成で整える。
...
- そのコンポーネントに閉じない処理のうち、bounded context や layer の意味を持つ共有処理は最も狭い `shared/` へ、文字列・数値・日付の単純変換のような純粋 utility は `packages/utils` へ寄せる。
- 1 ファイルに大量の関数や型を置かず、helper や型定義も独立した責務を持つなら別ファイルへ切り出す。
- 新しい型を足す前に、その場で使いたい意味が既存の ValueObject、Entity、shared contract、service result で表せないかを確認する。意味が増えない型 alias や薄い wrapper 型は追加しない。
...
- 複数 Repository で共有する mapper や helper のうち、永続化文脈を持つものは `src/repositories/shared/` に置く。Repository をまたいでも文脈を持たない純粋 utility は `packages/utils` へ寄せる。
- Repository 実装でも 1 ファイル 1 責務を守り、複数の保存処理、型定義、補助関数を単一ファイルへ詰め込まない。
- Prisma generated type や `packages/core` の契約で十分ならそのまま使い、query 専用の一時型や record mirror を安易に増やさない。
...
- repository test は対象 Repository ディレクトリ直下の `__tests__/` に必ず置き、永続化挙動を確認する。

## `apps/api` 連携ルール

- `apps/api` 固有のルールは `references/profiles/typescript/nest.md` を正本として扱う。
- `apps/api` は framework 依存の入口として保ち、Presenter / Builder / Mapper を使って response DTO を表現する。
- request DTO は `class-validator` と `ValidationPipe` を基本に扱い、DTO を domain model と兼用しない。
- response DTO や Builder / Presenter の入力型は transport 契約を表す意味があるときだけ追加し、既存 DTO や `packages/core` の result 型で十分なら流用する。
- `apps/api` の unit test も対象コンポーネントに最も近いディレクトリ直下の `__tests__/` に必ず置く。
- DB 接続付き API 結合テストは `__tests__/` へ混ぜず、既存 repo の結合テスト harness か `api/tests/http-integration/` 配下へ置く。

## 検証

- 変更した package / app の build、lint、型検査、test を順に実行する。
- repo に `verify` `check` `ci` のような包括 script があるなら、それを優先する。
- API を実装対象に含み、結合テスト実行環境がある場合は DB 接続付き API 結合テストも実行する。
