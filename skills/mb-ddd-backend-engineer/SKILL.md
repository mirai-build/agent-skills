...
4. 対象 repo が TypeScript モノレポなら [`references/profiles/typescript/monorepo.md`](./references/profiles/typescript/monorepo.md) を読み、`apps/api` が NestJS なら [`references/profiles/typescript/nest.md`](./references/profiles/typescript/nest.md) も読む。該当しない構成なら未対応として扱い、追加 profile が必要と伝える。
