# agent-skills

このリポジトリは、弊社のエンジニアが共用できる Codex App 用 Agent Skills を配置するためのリポジトリです。
再利用したい Skill を Git で管理し、ローカル環境や他リポジトリへ導入しやすい形で保管します。

## 同梱スキル

現在同梱している Skill は 14 個です。
最新の一覧は `python3 scripts/install_skills.py --list` でも確認できます。

- `skill-creator`
  - 表示名は `スキルクリエイター` です。
  - Codex Skill の作成と更新を支援する Skill です。
  - 明確な指示がない限り、Skill 本文や関連ドキュメントを日本語で作成する前提にカスタマイズしています。
- `mb-git-clean-branches`
  - 表示名は `ミライビルド gitブランチ掃除` です。
  - マージ済み Git ブランチの候補確認と安全な削除を支援する Skill です。
  - `skills/mb-git-clean-branches/` に同梱しています。
- `mb-git-rebase`
  - 表示名は `ミライビルド git rebase` です。
  - GitHub の PR 先判定と Bitbucket 向けの比較候補提示を使い、安全な rebase と競合解消を支援する Skill です。
  - `skills/mb-git-rebase/` に同梱しています。
- `mb-github-pr-comment-resolver`
  - 表示名は `ミライビルド GitHub PRコメントリゾルバー` です。
  - GitHub PR の未解決 review thread を分析し、方針確認後に修正、返信、resolve まで進める Skill です。
  - `skills/mb-github-pr-comment-resolver/` に同梱しています。
- `mb-pr-review`
  - 表示名は `ミライビルド PRレビュワー` です。
  - GitHub 上の PR またはローカルブランチ差分を、コーディングガイド確認と 3 パスレビューで P1/P2/P3 に分けて徹底レビューする Skill です。
  - `skills/mb-pr-review/` に同梱しています。
- `mb-ddd-architect`
  - 表示名は `ミライビルド DDDアーキテクト` です。
  - DDD に沿って不足情報を質問しながら、`docs/designs/` 配下の設計書を段階的に整備する Skill です。
  - `skills/mb-ddd-architect/` に同梱しています。
- `mb-ddd-architect-qa`
  - 表示名は `ミライビルド DDD設計Q&A` です。
  - `mb-ddd-architect` が整備した `docs/designs/` を読み、サービス設計、責務配置、DB 構造などの質問へ根拠つきで回答する Skill です。
  - 回答専用で、実装や設計書更新は行わず、`skills/mb-ddd-architect-qa/` に同梱しています。
- `mb-ddd-architect-reviewer`
  - 表示名は `ミライビルド DDD設計レビュワー` です。
  - 一般的な DDD の戦略設計・戦術設計の観点から、`docs/design/` または `docs/designs/` の設計書をレビューし、過不足、重大リスク、改善優先度を整理する Skill です。
  - `skills/mb-ddd-architect-reviewer/` に同梱しています。
- `mb-ddd-backend-engineer`
  - 表示名は `ミライビルド DDD バックエンドエンジニア` です。
  - DDD 設計書を基に、TypeScript モノレポの `packages/core` `packages/db` `apps/api` へバックエンド実装を落とし込む Skill です。
  - 初版は `core: TypeScript` `db: Prisma` `api: NestJS` をサポートし、`skills/mb-ddd-backend-engineer/` に同梱しています。
- `mb-react-fda-engineer`
  - 表示名は `ミライビルド React FDAエンジニア` です。
  - React の Feature Driven Architecture を前提に、既存 repo のディレクトリルールと利用ライブラリを尊重しつつフロントエンド実装を責務分離して進める Skill です。
  - 初版は TanStack Start を主対象に、Next App Router の profile とライブラリ方針も同梱し、`skills/mb-react-fda-engineer/` に同梱しています。
- `mb-implementation-qa`
  - 表示名は `ミライビルド 実装Q&A` です。
  - 現状実装を一次情報として棚卸しし、実装方針の相談や実装内容の確認へ、必要に応じて設計書も照合しながら根拠つきで答える Skill です。
  - 回答専用で、コード編集や設計書更新は行わず、`skills/mb-implementation-qa/` に同梱しています。
- `mb-code-quality-checker`
  - 表示名は `ミライビルド コード品質チェッカー` です。
  - 言語別ルールで整形、lint、型検査、テストの実行コマンドを見極め、失敗時はコード修正まで進める Skill です。
  - 初版は TypeScript をサポートし、`skills/mb-code-quality-checker/` に同梱しています。
- `mb-document-downloader`
  - 表示名は `ミライビルド 文書ダウンローダー` です。
  - 対象 SaaS の文書ページを取得し、Markdown へ変換してローカル保存する Skill です。
  - 初期 provider は Confluence で、`ATLASSIAN_TOKEN` を必須にしつつ `--provider` で対象 SaaS を切り替えられる形にそろえ、`skills/mb-document-downloader/` に同梱しています。
- `mb-repo-quality-reviewer`
  - 表示名は `ミライビルド リポジトリ品質レビュワー` です。
  - リポジトリ全体の構造、責務分離、可読性、保守性、テスト容易性、過剰設計を総合レビューする Skill です。
  - 初版は TypeScript と Python を重点サポートし、`skills/mb-repo-quality-reviewer/` に同梱しています。

## ディレクトリ構成

- `skills/`: 配布対象の Skill 本体を配置します。
- `scripts/install_skills.py`: このリポジトリ内の Skill をインストールするためのスクリプトです。

## インストール方法

### 1. Codex App で全部インストールする

Codex App でこのリポジトリを開いて、チャットで `全部インストールして` と依頼すれば完了です。
Codex が `scripts/install_skills.py --mode home --all` を実行し、home 配下へまとめてインストールします。

### 2. ローカル home へインストールする

Codex App に自動検出させたい場合は、home 配下へインストールします。
現行のインストール先は `~/.agents/skills` です。
以前の `~/.codex/skills` は旧配置なので、見えない Skill がある場合は `~/.agents/skills` へ入れ直して下さい。

```bash
python3 scripts/install_skills.py --mode home \
  --skill skill-creator \
  --skill mb-git-clean-branches \
  --skill mb-git-rebase \
  --skill mb-github-pr-comment-resolver \
  --skill mb-pr-review \
  --skill mb-ddd-architect \
  --skill mb-ddd-architect-qa \
  --skill mb-ddd-architect-reviewer \
  --skill mb-ddd-backend-engineer \
  --skill mb-react-fda-engineer \
  --skill mb-implementation-qa \
  --skill mb-code-quality-checker \
  --skill mb-document-downloader \
  --skill mb-repo-quality-reviewer
```

すべての Skill をまとめてインストールする場合:

```bash
python3 scripts/install_skills.py --mode home --all
```

home へインストールしたあとは、Codex App を再起動して Skill を読み込んで下さい。

### 3. 他リポジトリへインストールする

他リポジトリの `.agents/skills/` 配下へコピーして、リポジトリと一緒に管理したい場合はこちらを使います。

```bash
python3 scripts/install_skills.py --mode repo --repo-path /path/to/target-repo \
  --skill skill-creator \
  --skill mb-git-clean-branches \
  --skill mb-git-rebase \
  --skill mb-github-pr-comment-resolver \
  --skill mb-pr-review \
  --skill mb-ddd-architect \
  --skill mb-ddd-architect-qa \
  --skill mb-ddd-architect-reviewer \
  --skill mb-ddd-backend-engineer \
  --skill mb-react-fda-engineer \
  --skill mb-implementation-qa \
  --skill mb-code-quality-checker \
  --skill mb-document-downloader \
  --skill mb-repo-quality-reviewer
```

すべての Skill をまとめてインストールする場合:

```bash
python3 scripts/install_skills.py --mode repo --repo-path /path/to/target-repo --all
```

repo へのインストールは、共有やバージョン管理を目的とした運用向けです。
Codex は repo の `.agents/skills/` も自動検出しますが、反映が見えないときは Codex App の再起動を行って下さい。

### 4. インストール対象を一覧表示する

```bash
python3 scripts/install_skills.py --list
```

### 5. 既存 Skill を置き換える / 事前確認する

同名 Skill がすでに入っている環境へ再インストールする場合は `--force` を付けます。
実際にはコピーせず、対象とインストール先だけ確認したい場合は `--dry-run` を使います。

```bash
python3 scripts/install_skills.py --mode home --all --dry-run
```

```bash
python3 scripts/install_skills.py --mode home --skill mb-document-downloader --force
```

## 補足

- インストーラは別 Skill として増やさず、決定的に動作するスクリプトとして提供しています。
- Skill を追加または削除したら `skills/` 配下の構成に合わせて、この README の一覧と導入例も更新して下さい。
- 新しい Skill には必ず `license.txt` を追加し、`SKILL.md` の frontmatter に `license` を記載して下さい。
