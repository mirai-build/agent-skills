...
- 基本構成ごとに、ナビゲーション方針、画面の大分類、レイアウトの基本型、利用端末ごとのレスポンシブ方針、共通 UI 領域を追えるか。
...
- 同じ画面を複数ジャーニーで共有する場合も、画面を複製せず 1 行で管理できているか。
...
- 各画面で、参照する基本構成、目的、主 actor、対応ジャーニー、代表的な開始条件と完了条件、主要状態、関連 context を追えるか。
- 同じ画面を複数ジャーニーで共有する場合も、画面を複製せず、ジャーニー別の利用文脈と完了条件を整理できているか。
- 基本構成で定義した共通 UI 領域のうち、この画面で使うものと、必要なら意図的に外すものを説明できるか。
- 画面内 UI の一覧と、必要な UI ごとの詳細が、画面全体の責務と矛盾せず整理されているか。
- 必要な端末差分やレスポンシブ時の変化が、`basic_layouts` または `screen_details` で追えるか。
- `入口と出口` のような遷移定義を持ち込まず、遷移責務を `navigation_map` へ寄せられているか。
- 表示責務と操作責務が、画面一覧の簡単な説明と矛盾していないか。
- 確定事項、仮置き、未確定事項が区別されているか。
